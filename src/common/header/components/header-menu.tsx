        ...
        <div className='flex'>
            ...
            <div className='grid gap-10'>
              {dataMenu.dataleft.map((item) => (
                <Link
                  to={item.link}
                  key={item.id}
                  className={`flex items-center gap-4 text-black text-opacity-50 cursor-pointer ${
                    item.id === dataMenu.dataleft.length - 1
                      ? 'border-blue-600 font-bold justify-start text-[12px] outline-1 outline-blue-600 p-2 rounded-t-sm'
                      : 'text-[16px]'
                  }`}
                >
                  <img src={item.img} alt="" className='max-w-10 rounded-2xl' />
          ...
          <div className='mt-4 ml-8'>
            <h1 className='text-[12px] h-[3em]'>
              ...
              >
                {dataMenu.viewNow}
                {/* Nếu icon là FontAwesome thì mở comment dưới */}
                {/* <FontAwesomeIcon icon={dataMenu.icon} /> */}
              </Link>
            </h1>
